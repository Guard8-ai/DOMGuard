# Privacy Policy for DOMGuard Inspector

**Last updated:** January 14, 2026

## Overview

DOMGuard Inspector is a browser extension that helps developers visually identify and select DOM elements on webpages. This privacy policy explains how we handle user data.

## Data Collection

**We do not collect, store, or transmit any personal data.**

The extension operates entirely locally on your device. No information is sent to external servers.

## What the Extension Does

- Injects visual highlighting scripts into webpages you choose to inspect
- Detects element information (tag names, CSS selectors, dimensions) when you hover
- Copies CSS selectors to your clipboard when you click

## Permissions Explained

| Permission | Purpose |
|------------|---------|
| `activeTab` | Inject inspector into the current tab when you activate it |
| `scripting` | Add visual highlighting overlay to webpages |
| `clipboardWrite` | Copy selectors to your clipboard |
| `<all_urls>` | Allow inspection on any webpage you're developing |

## Third Parties

We do not share any data with third parties because we do not collect any data.

## Changes to This Policy

If we update this privacy policy, we will post the changes here with an updated revision date.

## Contact

For questions about this privacy policy, please open an issue at:
https://github.com/Guard8-ai/DOMGuard/issues

---

Guard8.ai - High Quality Code -> Great Results
